import sys, os, shutil, sqlite3, time, winreg
...
class CustomItem(QGraphicsItem):
    def __init__(self, x, y, Onleft, name, child, eventValue, DBfullname, DBname, view):
        super().__init__()
        self.DBfullname = DBfullname
        self.DBname = DBname
        self.view = view
    ...
    def mousePressEvent(self, event):
        ...
        try:
            ...
            if self.selecteditem.scenePos().x() == self.scenePos().x() \
            and self.selecteditem.scenePos().y() + 10 == self.scenePos().y(): self.selecteditem_move = True
    ...
    def mouseReleaseEvent(self, event):
        ...
        self.connect = sqlite3.connect(self.DBfullname)
...
class GraphicView(QGraphicsView):
    def __init__(self):
        ...
        self.setBackgroundBrush(QtGui.QBrush(QtGui.QColor(30, 30, 30), Qt.Dense1Pattern))
...
class paramWindow(QWidget):
    def __init__(self, tree, eventValue, parent, child, DBname, DBfullname, child2, view):
    ...
    def ADDitem(self):
        ...
        if self.eventValue == 1:
            self.posx = 0
            self.posy = 0
            self.item = CustomItem(self.posx, self.posy, True, self.text, False, self.eventValue, self.DBfullname, self.DBname, self.view)
        ...
        if self.eventValue == 2:
            ...
            self.item = CustomItem(self.posx, self.posy, False, self.text, True, self.eventValue, self.DBfullname, self.DBname, self.view)
            scene.addItem(self.item)
            scene.lineConnect(self.mainItem.childItems()[0], self.item.childItems()[0])
        if self.eventValue == 3:
            query = 'SELECT graphicitem FROM "{table}" WHERE type = ? AND parent = ?'
            self.cursor.execute(query.format(table=self.DBname), (2, self.parent,))
            ...
            self.posx = self.mainItem.pos().x() + 100
            self.posy = self.mainItem.pos().y()
            self.item = CustomItem(self.posx, self.posy, False, self.text, False, self.eventValue, self.DBfullname, self.DBname, self.view)
            scene.addItem(self.item)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle('CMconstruct')
        self.setWindowIcon(QtGui.QIcon('icon.jpg'))
    ...
    #Создание DB
    def DBLoader(self):
        self.DBfullname = 'CMconstruct.db'
    ...
    def DBinsert(self, eventValue):
            ...
            if eventValue == 1:
                ...
                self.window = paramWindow(tree, eventValue, self.parent, self.child,
                                          self.DBname, self.DBfullname, self.child2, self.plain6)
                self.window.show()
            if eventValue == 2:
    ...
    def saveAsFile(self):
        ...
        try:
            file = open(name[0], 'w')
            BDname, BDname_reverse = '', ''
            self.filepath = name[0]

            for i in reversed(range(len(name[0]))):
                if name[0][i] != '/': BDname_reverse +=  name[0][i]
                else: break
            for i in reversed(BDname_reverse): BDname += i
            ...
            self.connect.commit()
            self.DBname = BDname[:-3]
            self.DBfullname = BDname
    ...
    def openFile(self):
        ...
        try:
            ...
            for i in range(len(data)):
                ...
                if type == 1:
                    newitem = CustomItem(posx, posy, True, name, False, 1, self.DBfullname, self.DBname, self.plain6)
                    self.plain6.scene.addItem(newitem)
                if type == 2:
                    newitem = CustomItem(posx, posy, False, name, True, 2, self.DBfullname, self.DBname, self.plain6)
                    self.plain6.scene.addItem(newitem)
                if type == 3:
                    newitem = CustomItem(posx, posy, False, name, False, 3, self.DBfullname, self.DBname, self.plain6)
                    self.plain6.scene.addItem(newitem)
